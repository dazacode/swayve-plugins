...
Which manifest *format* the file uses. `5` as of the `personal_library_push`
capability (`4` as of `session_capture`, `3` as of `personal_library`, `2` as
of `artist_activity` before it); a manifest declaring `6` is rejected by a
build that only understands up to `5`, rather than optimistically parsed.

This is the first thing the host checks, because it determines whether the rest
of the file can be interpreted at all. A future `schemaVersion: 6` might mean
...
below it: a build that understands `schemaVersion: 5` also reads a
